...
class HookRendered extends Hook {
    ...
    static device_handler(message) {
        ...
        const nodes = document.querySelectorAll(`[data-id="${message.id._serialized}"]`);
        if (!nodes || nodes.length !== 1) return;

        const root = nodes[0];
        // Tenta achar a caixa de texto da mensagem
        const parts = root?.childNodes?.[0]?.childNodes ? Array.from(root.childNodes[0].childNodes) : [];
        const message_box = parts.find((el) => (el?.innerText || '').includes(':')) || parts[parts.length - 1];
        if (!message_box || !message_box.childNodes || message_box.childNodes.length < 1) return;

        const insert_into = message_box.childNodes[message_box.childNodes.length - 1];
        if (!insert_into || typeof insert_into.querySelector !== 'function') return;

        // Evita duplicar
        if (insert_into.querySelector('svg')) return;

        const wrapper = document.createElement('span');
        wrapper.style.display = 'inline-flex';
        wrapper.style.verticalAlign = 'middle';
        wrapper.style.marginRight = '4px';
        wrapper.innerHTML = get_svg_by_id(message.id.id);

        insert_into.prepend(wrapper.firstElementChild || wrapper);
    }
}
